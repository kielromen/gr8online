...
if (Not (IsEmpty(reportid) OR (Len(reportid) = 0))) then
    ...
    Set objectFactory = CreateObject("CrystalReports115.ObjectFactory.1")
	...
	' Try to load report source from session
        ...
        if (IsEmpty(HTMLViewer.ReportSource)) then
            errorHeader = "L_SessionExpired"
            errorMessage = "L_PleaseRefresh"
        end if
    end if

    if (IsEmpty(errorHeader)) then
        ...
        Response.Write "<script language=""javascript"" src=""js/previewerror.js""></script>"
        Response.Write "<script>writeError(" & errorHeader & "," & errorMessage & ");</script>"
    end if

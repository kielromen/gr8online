...
Public Class OfficialReceipt
    ...
    Dim ColumnPK As String = "OR_No"
    Dim ColumnID As String = "TransID"
    Dim DBTable As String = "tblCollection_OR"
    ...
    Private Sub OfficialReceipt_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        If Not IsPostBack Then
            Session("ID") = ""
            Session("CopyFromID") = ""
            Session("CopyID") = ""
    ...
    Private Sub btnEdit_Command(sender As Object, e As CommandEventArgs) Handles btnEdit.Command
        EnableControl(True)
        btnSearch.Attributes("disabled") = "disabled"
        btnNew.Attributes("disabled") = "disabled"
        btnEdit.Attributes("disabled") = "disabled"
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCollection_OR  WHERE OR_No > '" & Session("Transno") & "' ORDER BY OR_No ASC "
            SQL.ReadQuery(query)
    ...
    Private Sub btnPrev_Click(sender As Object, e As EventArgs) Handles btnPrev.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCollection_OR  WHERE OR_No < '" & Session("Transno") & "' ORDER BY OR_No DESC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadTransaction(TransID)
    ...
    Private Sub LoadTransaction(ByVal ID As String)
        ...
        query = " SELECT  TransID, OR_No, tblCollection_OR.VCECode, Name, TransDate, Amount, Remarks, PaymentType, DepositTo, " &
                "          tblCollection_OR.Status, CheckDate, BankRef, CheckRef " &
        ...
        If SQL.SQLDR.Read Then
            ...
            ORNo = SQL.SQLDR("OR_No").ToString
            Session("Transno") = SQL.SQLDR("OR_No").ToString
            txtTrans_Num.Text = ORNo
    ...
    Private Sub LoadEntry(ByVal ORNo As Integer)
        ...
        query = " SELECT ID, JE_No, View_GL.BranchCode, View_GL.AccntCode, AccountTitle, View_GL.VCECode, View_GL.VCEName, Debit, Credit, Particulars, CostID, RefNo, VAtType   " &
                ...
                " WHERE JE_No = (SELECT  JE_No FROM tblJE_Header WHERE Upload = 0 AND RefType = 'OR' AND RefTransID = " & ORNo & ") " &
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If (Page.IsValid) Then
            ...
            If Session("TransID") = "" Then
                ...
                If TransAuto Then
                    ORNo = GenerateTransNum(TransAuto, ModuleID, ColumnPK, DBTable)
                Else
                    ORNo = txtTrans_Num.Text
                End If
                txtTrans_Num.Text = ORNo
    ...
    Public Sub Save()
        ...
        insertSQL = " INSERT INTO " &
                        " tblCollection_OR (TransID,  OR_No, TransDate, VCECode, PaymentType, DepositTo, Amount,  CheckRef, BankRef, CheckDate, Remarks, TransAuto, WhoCreated) " &
                        " VALUES(@TransID, @OR_No, @TransDate, @VCECode, @PaymentType, @DepositTo, @Amount, @CheckRef, @BankRef, @CheckDate, @Remarks, @TransAuto, @WhoCreated)"
        SQL.FlushParams()
        SQL.AddParam("@TransID", TransID)
        SQL.AddParam("@OR_No", ORNo)
    ...
    Private Sub Update()
        ...
        activityStatus = True
        updateSQL = " UPDATE tblCollection_OR  " &
                        " SET     OR_No = @OR_No, TransDate = @TransDate, PaymentType = @PaymentType, DepositTo = @DepositTo," &
        ...
        JETransiD = LoadJE("OR", Session("TransID").ToString)
        If JETransiD = 0 Then
            insertSQL = " INSERT INTO " &
                        " tblJE_Header (AppDate, RefType, RefTransID, Book, TotalDBCR, Remarks, WhoCreated) " &
                        " VALUES(@AppDate, @RefType, @RefTransID, @Book, @TotalDBCR, @Remarks, @WhoCreated)"
    ...
    Private Function IfExist(ByVal ID As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblCollection_OR WHERE OR_No ='" & ID & "'  "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            Return True
        Else

...
Public Class ProvisionalReceipt
    ...
    Dim DBTable As String = "tblCollection_PR"
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCollection_PR  WHERE PR_No > '" & Session("Transno") & "' ORDER BY PR_No ASC "
            SQL.ReadQuery(query)
    ...
    Private Sub btnPrev_Click(sender As Object, e As EventArgs) Handles btnPrev.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCollection_PR  WHERE PR_No < '" & Session("Transno") & "' ORDER BY PR_No DESC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadTransaction(TransID)
    ...
    Private Sub LoadTransaction(ByVal ID As String)
        ...
        query = " SELECT  TransID, PR_No, tblCollection_PR.VCECode, Name, TransDate, Amount, Remarks, PaymentType, DepositTo," &
                "          tblCollection_PR.Status, CheckDate, BankRef, CheckRef " &
        ...
        If SQL.SQLDR.Read Then
            ...
            ARNo = SQL.SQLDR("PR_No").ToString
            Session("Transno") = SQL.SQLDR("PR_No").ToString
    ...
    Public Sub Save()
        ...
        insertSQL = " INSERT INTO " &
                        " tblCollection_PR (TransID,  PR_No, TransDate, VCECode, PaymentType,DepositTo, Amount,  CheckRef, BankRef, CheckDate, Remarks, TransAuto, WhoCreated) " &
                        " VALUES(@TransID, @PR_No, @TransDate, @VCECode, @PaymentType, @DepositTo, @Amount, @CheckRef, @BankRef, @CheckDate, @Remarks, @TransAuto, @WhoCreated)"
        SQL.FlushParams()
        SQL.AddParam("@TransID", TransID)
        SQL.AddParam("@PR_No", ARNo)
    ...
    Private Sub Update()
        ...
        activityStatus = True
        updateSQL = " UPDATE tblCollection_PR  " &
                        " SET     PR_No = @PR_No, TransDate = @TransDate, PaymentType = @PaymentType, DepositTo = @DepositTo," &
        ...
        JETransiD = LoadJE("PR", Session("TransID").ToString)
        If JETransiD = 0 Then
            insertSQL = " INSERT INTO " &
                        " tblJE_Header (AppDate, RefType, RefTransID, Book, TotalDBCR, Remarks, WhoCreated) " &
                        " VALUES(@AppDate, @RefType, @RefTransID, @Book, @TotalDBCR, @Remarks, @WhoCreated)"
    ...
    Private Function IfExist(ByVal ID As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblCollection_PR WHERE PR_No ='" & ID & "'  "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            Return True
        Else

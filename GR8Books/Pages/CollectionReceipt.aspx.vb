...
Public Class CollectionReceipt
    ...
    Dim ModuleID As String = "CR"
    Dim ColumnPK As String = "CR_No"
    Dim ColumnID As String = "TransID"
    Dim DBTable As String = "tblCollection_CR"
    ...
    Private Sub CollectionReceipt_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        If Not IsPostBack Then
            Session("ID") = ""
            Session("CopyFromID") = ""
            Session("CopyID") = ""
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCollection_CR  WHERE CR_No > '" & Session("Transno") & "' ORDER BY CR_No ASC "
            SQL.ReadQuery(query)
    ...
    Private Sub btnPrev_Click(sender As Object, e As EventArgs) Handles btnPrev.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCollection_CR  WHERE CR_No < '" & Session("Transno") & "' ORDER BY CR_No DESC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadTransaction(TransID)
    ...
    Private Sub LoadTransaction(ByVal ID As String)
        ...
        query = " SELECT  TransID, CR_No, tblCollection_CR.VCECode, Name, TransDate, Amount, Remarks, PaymentType,DepositTo, " &
                "          tblCollection_CR.Status, CheckDate, BankRef, CheckRef " &
        ...
        If SQL.SQLDR.Read Then
            ...
            CRNo = SQL.SQLDR("CR_No").ToString
            Session("Transno") = SQL.SQLDR("CR_No").ToString
            txtTrans_Num.Text = CRNo
    ...
    Private Sub LoadEntry(ByVal ID As Integer)
        ...
        query = " SELECT ID, JE_No, View_GL.BranchCode, View_GL.AccntCode, AccountTitle, View_GL.VCECode, View_GL.VCEName, Debit, Credit, Particulars, CostID, RefNo , VatType  " &
                ...
                " WHERE JE_No = (SELECT  JE_No FROM tblJE_Header WHERE Upload = 0 AND RefType = 'CR' AND RefTransID = " & ID & ") " &
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If (Page.IsValid) Then
            ...
            If Session("TransID") = "" Then
                ...
                If TransAuto Then
                    CRNo = GenerateTransNum(TransAuto, ModuleID, ColumnPK, DBTable)
                Else
                    CRNo = txtTrans_Num.Text
                End If
                txtTrans_Num.Text = CRNo
    ...
    Public Sub Save()
        ...
        insertSQL = " INSERT INTO " &
                        " tblCollection_CR (TransID,  CR_No, TransDate, VCECode, PaymentType, DepositTo, Amount,  CheckRef, BankRef, CheckDate, Remarks, TransAuto, WhoCreated) " &
                        " VALUES(@TransID, @CR_No, @TransDate, @VCECode, @PaymentType,@DepositTo, @Amount, @CheckRef, @BankRef, @CheckDate, @Remarks, @TransAuto, @WhoCreated)"
        SQL.FlushParams()
        SQL.AddParam("@TransID", TransID)
        SQL.AddParam("@CR_No", CRNo)
    ...
    Private Sub Update()
        ...
        activityStatus = True
        updateSQL = " UPDATE tblCollection_CR  " &
                        " SET     CR_No = @CR_No, TransDate = @TransDate, PaymentType = @PaymentType, DepositTo = @DepositTo," &
        ...
        JETransiD = LoadJE("CR", Session("TransID").ToString)
        If JETransiD = 0 Then
            insertSQL = " INSERT INTO " &
                        " tblJE_Header (AppDate, RefType, RefTransID, Book, TotalDBCR, Remarks, WhoCreated) " &
                        " VALUES(@AppDate, @RefType, @RefTransID, @Book, @TotalDBCR, @Remarks, @WhoCreated)"
    ...
    Private Function IfExist(ByVal ID As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblCollection_CR WHERE CR_No ='" & ID & "'  "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            Return True
        Else

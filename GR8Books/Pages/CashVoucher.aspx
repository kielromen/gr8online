<%@ Page Title="Cash Voucher" MaintainScrollPositionOnPostback="true" Language="vb" AutoEventWireup="false" MasterPageFile="~/Master/Dashboard.Master" CodeBehind="CashVoucher.aspx.vb" Inherits="GR8Books.CashVoucher" %>
        ...
        $(document).ready(function () {
            ...
            $(".txtAccntTitle_Entry").autocomplete({
                source: function (request, response) {
                    $.ajax({
                        url: "<%= ResolveUrl("CashVoucher.aspx/ListAccountTitle") %>",
            ...
            $(".txtName_Entry").autocomplete({
                source: function (request, response) {
                    $.ajax({
                        url: "<%= ResolveUrl("CashVoucher.aspx/ListVCE") %>",
                        data: "{'prefix': '" + request.term + "'}",
                        dataType: "json",
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
            ...
            $("#<%=ddlTaxType.ClientID%>").change(function () {
                ...
                $.ajax({
                    url: "<%= ResolveUrl("CashVoucher.aspx/LoadTaxPercent") %>",
                    data: "{'prefix': '" + taxcode + "'}",
                    dataType: "json",
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
                        var percent = data.d;
                        if (percent != "") {
                            $("#<%=txtTPercent.ClientID%>").val(percent);
                            ...
                            var taxamount = parseFloat($("#<%=txtTNetAmount.ClientID%>").val().replace(/,/g, "")) * percent;
                            if (isNaN(taxamount)) {
                                taxamount = 0;
                            }
            ...
            $("#<%=ddlETaxType.ClientID%>").change(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        var percent = data.d;
                        if (percent != "") {
                            $("#<%=txtEPercent.ClientID%>").val(percent);
                            percent = parseFloat(percent.replace("%", "")) / 100;
                            var amount = parseFloat($("#<%=txtTAmount.ClientID%>").val().replace(/,/g, ""));
                            if (amount > 0 && parseFloat($("#<%=txtTNetAmount.ClientID%>").val().replace(/,/g, "")) == 0) {
                                amount = amount / (1 + percent);

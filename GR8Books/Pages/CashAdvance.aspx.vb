...
Public Class CashAdvance
    ...
    Private Sub CashAdvance_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        If Not IsPostBack Then
            Session("ID") = ""
        End If
    End Sub
    ...
    Public Sub Save()
        ...
        insertSQL = " INSERT INTO " &
                        " tblCA (TransID, CA_No, VCECode, TransDate, AccountCode, Amount,  Remarks, CostID, Status, WhoCreated, TransAuto ) " &
                        " VALUES (@TransID, @CA_No, @VCECode, @TransDate, @AccountCode, @Amount, @Remarks, @CostID, @Status, @WhoCreated, @TransAuto)"
        SQL.FlushParams()
    ...
    Private Sub UpdateCA()
        ...
        activityStatus = True
        updateSQL = " UPDATE tblCA  " &
                        " SET    CA_No = @CA_No, VCECode = @VCECode, TransDate = @TransDate, Amount = @Amount," &
    ...
    Private Sub LoadTransaction(ByVal ID As String)
        ...
        If SQL.SQLDR.Read Then
            ...
            Session("Transno") = SQL.SQLDR("CA_No").ToString
            txtTrans_Num.Text = CANO
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCA  WHERE CA_No > '" & Session("Transno") & "' ORDER BY CA_No ASC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadTransaction(TransID)
    ...
    Private Sub btnPrev_Click(sender As Object, e As EventArgs) Handles btnPrev.Click
        If Session("Transno") <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblCA  WHERE CA_No < '" & Session("Transno") & "' ORDER BY CA_No DESC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadTransaction(TransID)
